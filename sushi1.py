...
# SECCIÓN 2: CALCULADORA
    ...
    calorias_disponibles = st.number_input(
        "Calorías disponibles para la comida:", min_value=50, max_value=5000, step=50
    )
    cuantas_piezas_de_sushi = st.number_input(
        "Cuantas piezas de sushi he comido?:", min_value=1, max_value=100, step=1
    )
    tipo_sushi = st.selectbox(
        "Seleccione el tipo de sushi:",
    ...
    if calorias_disponibles > 0 and calorias_disponibles < 5000:
        calorias_por_pieza = calorias_sushi[tipo_sushi]
        piezas_posibles = calorias_disponibles // calorias_por_pieza

        st.success(
            "Puedes comer aproximadamente **5 piezas de sushi hasta 30 piezas de sushi**."
        )
        st.info("Cada pieza de sushi contiene aproximadamente 50 calorias.**")

        st.markdown("### Recomendaciones:")
    if cuantas_piezas_de_sushi > piezas_posibles:
        st.warning(
            "Estás comiendo bastante sushi ¡Considera acompañar con ensalada o té verde!"
        )
    elif cuantas_piezas_de_sushi <= piezas_posibles and cuantas_piezas_de_sushi >=piezas_posibles:
